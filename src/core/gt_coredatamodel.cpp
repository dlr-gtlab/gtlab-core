                                   ...
                                   QList<GtObject*> const& projectData)
{
    ...
    project->m_isOpen = true;

    // notify model
    endInsertRows();

    // update current project in application
    if (!gtApp->setCurrentProject(project))
    {
        beginRemoveRows(objIndex, 0, projectData.size() - 1);
        qDeleteAll(project->findDirectChildren<GtObject*>());
        project->m_isOpen = false;
        endRemoveRows();
        return false;
    }
...
GtCoreDatamodel::closeProject(GtProject* project)
{
    ...
    // Clear the active project while its data is still alive. This gives
    // observers a chance to release references before the objects are deleted.
...
bool
GtCoreDatamodel::switchProject(GtProject* project)
{
    if (!project)
    {
        return false;
    ...
    GtProject* current = currentProject();
    ...
    if (current && !closeProject(current))
    {
        return false;
    }
...
GtCoreDatamodel::newProject(GtProject* project, bool doOpen)
{
    ...
    if (doOpen)
    {
        if (openProject(project->objectName()))
        {
            return true;
        }

        const QModelIndex projectIndex = indexFromObject(project);
        beginRemoveRows(QModelIndex(), projectIndex.row(), projectIndex.row());
        project->disconnectFromParent();
        m_session->save();
        endRemoveRows();
        return false;
    }
