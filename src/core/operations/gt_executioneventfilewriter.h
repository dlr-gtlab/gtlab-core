...
public:
    ...
    /**
     * @brief Returns whether the event file was opened successfully.
     */
    bool isOpen() const;
    /**
    ...
    /**
     * @brief Returns whether opening, writing, or flushing the event file failed.
     */
    bool hasError() const;
    /**
     * @brief Returns the first event-file I/O error, or an empty string on success.
     */
    QString errorString() const;

public slots:
    /**
     * @brief Appends one compact JSON event record and its terminating newline.
     */
    void writeEvent(GtExecutionEvent const& event);

signals:
    /**
     * @brief Emitted once when an event record can no longer be persisted.
     */
