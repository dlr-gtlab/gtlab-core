...
public:
    /**
     * @brief Creates an empty event for Qt metatype construction only.
     */
    GtExecutionEvent();
    ...
    /**
     * @brief Returns the identity shared by all events of this execution.
     */
    GtExecutionId const& executionId() const noexcept;
    /**
     * @brief Returns the strictly increasing, zero-based per-execution sequence.
     */
    quint64 sequence() const noexcept;
    /**
     * @brief Returns the domain-specific type key describing this event.
     */
    QString const& eventType() const noexcept;
    /**
     * @brief Returns the JSON payload, or an undefined value when no payload was set.
     */
