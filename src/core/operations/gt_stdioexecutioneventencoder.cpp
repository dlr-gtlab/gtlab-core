...
namespace
{
    ...
    QJsonObject commonRecord(QString const& kind,
...
GtStdioExecutionEventEncoder::GtStdioExecutionEventEncoder(
    ...
    QObject(parent),
    m_executionId(std::move(executionId)), m_output(output)
{
}

bool
GtStdioExecutionEventEncoder::isTerminal() const noexcept
{
...
void
GtStdioExecutionEventEncoder::encodeEvent(GtExecutionEvent const& event)
{
    QJsonObject record =
        commonRecord(QStringLiteral("event"), event.executionId());
    record.insert(QStringLiteral("sequence"),
                  static_cast<qint64>(event.sequence()));
    record.insert(QStringLiteral("eventType"), event.eventType());
    record.insert(QStringLiteral("payloadEncoding"), QStringLiteral("json"));
    record.insert(QStringLiteral("payload"), event.payload().isUndefined()
                                                 ? QJsonValue::Null
                                                 : event.payload());
...
bool
GtStdioExecutionEventEncoder::encodeResult(GtObject const* result)
{
...
bool
GtStdioExecutionEventEncoder::encodeFailure(QString errorCode, QString message,
...
bool
GtStdioExecutionEventEncoder::writeRecord(QJsonObject record, bool terminal)
{
    std::lock_guard<std::mutex> lock(m_mutex);
    if (m_terminal || m_outputFailed ||
