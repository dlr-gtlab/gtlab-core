...
GtContour::GtContour() : m_polygonCalculation(false) {}
...
QList<QPolygonF> GtContour::calcContour(GtTable *table, const QString &param,
  ...
  return calcContour(table, param, isoValues, calcPolygons, returnPolygonsOnly,
                     check)
      .value(isoValue);
}
...
bool GtContour::setData(GtTable *table, const QString &paramName, int iDim,
                        int jDim) {
  ...
  for (int i = 0; i < iDim; i++) {
    for (int j = 0; j < jDim; j++) {
      newXvalues.append(axis0Min + i * (axis0Max - axis0Min) / (iDim - 1));

      newYvalues.append(axis1Min + j * (axis1Max - axis1Min) / (jDim - 1));

      double res = 0.0;

      try {
        res = table->getValue2D(paramName, newXvalues.at(counter),
  ...
  m_table.addAxis("phi", "dimensionless phi axis", "[-]", GtTableAxis::E_LINEAR,
                  GtTableAxis::I_LINEAR, GtTableAxis::E_LINEAR, phiValues);

  m_table.addAxis("xi", "dimensionless xi axis", "[-]", GtTableAxis::E_LINEAR,
                  GtTableAxis::I_LINEAR, GtTableAxis::E_LINEAR, xiValues);
...
QStringList GtContour::isoValues() const { return m_isoValues; }

bool GtContour::isValid(GtTable *t, const QString &paramName) const {
