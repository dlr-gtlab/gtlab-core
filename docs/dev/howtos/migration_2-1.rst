...
While we strive to avoid API breaking changes between minor versions, some changes to the API may be unavoidable and would otherwise greatly complicate needed changes or obfuscate the use of the API for the developers.

Changes that may require additional information or an in-depth migration plan are listed below:
...
.. list-table::
   ...
   * - **API**
     - **Since**
     - **Replacement / Notes**
   * - Use of ``GtGraphicsView``, ``GtGrid``, and ``GtRuler``.
   
       Removed ``GtGrid::setHorizontalRuler(GtRuler*)``, ``GtGrid::setVerticalRuler(GtRuler*)``, and ``GtGrid::paintRuler(GtRuler*)``
     - 2.1.0
     - :ref:`Migration to GTlab 2.1 - Graphics API <label_migration_2-1_graphics_api>`
   * - Removed ``GtGrid::setShowGridPoints(bool)`` and ``GtGrid::setGridPointColor(QColor)``
     - 2.1.0
     - No replacement planned. Previous implementations did not provide any functionality.
   * - Removed ``GtGrid::setGridScaleFactor(int)``
     - 2.1.0
     - Use ``GtGrid::setCurrentZoom(double)`` instead (e.g. using ``GtGraphicsView::zoom`` as the parameter).
   * - Removed ``GtRuler::buffer()`` (previosuly ``public``)
     - 2.1.0
     - Use ``GtRuler::cache`` instead (now ``protected``).
   * - Removed ``GtRuler::needsRepaint()``
     - 2.1.0
     - Use ``GtRuler::needsRepaint(QRectF, QTransform)`` instead.
   * - Removed ``GtRuler::setNeedsRepaint()``
     - 2.1.0
     - Use ``GtRuler::invalidate()`` instead.
