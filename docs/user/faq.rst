...
To make additional scripts accessible, add the directory containing them to Python's search path. For example:
...
.. image:: scripting/images/python_nodes_faq1.png
  :align: center
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
A: Use the node ports for data exchange. Only compatible port types can be connected.
On the input side, the Python node can handle the available GTlab data types.
Many of them expose their value through a ``value()`` method or a similar accessor.
For direct Python object transfer, use ``intelli::ByteArrayData`` and Python pickle-compatible objects.
